...
Future<void> _build(PluginTarget platform, Directory tmpDir) =>
    Github.logGroupAsync('Build libsodium-${platform.name} artifact', () async {
      final artifactDir = await Github.env.runnerTemp
          .subDir('libsodium-${platform.name}')
          .create();

      try {
        await platform.build(extractDir: tmpDir, artifactDir: artifactDir);
      } on Exception {
        final logFile = tmpDir.subDir('libsodium-stable').subFile('config.log');
...
Future<void> _getArchive(Directory downloadDir, Uri downloadUrl) =>
    Github.logGroupAsync('Download, verify and extract $downloadUrl', () async {
      final httpClient = HttpClient();
      try {
        final archive = await httpClient.download(downloadDir, downloadUrl);
