...
Future<void> main() => Github.runZoned(() async {
  ...
  final downloadUrls = PluginTargets.allTargets
      .map((t) => t.downloadUrl)
      .toSet();
  ...
  try {
    for (final downloadUrl in downloadUrls) {
      Github.logInfo('Getting last modified header for: $downloadUrl');
  ...
  Github.logNotice('At least one upstream archive has been modified!');
  await Github.env.setOutput('modified', true);
});

String _buildLastModifiedFile(Map<Uri, String> lastModifiedMap) {
  final lines =
      lastModifiedMap.entries.map((e) => '${e.key} - ${e.value}\n').toList()
        ..sort();
