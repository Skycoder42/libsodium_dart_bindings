...
void main() {
  ...
  setUp(() {
    ...
    sut = XofTurboshake128JS(mockSodium.asLibSodiumJS);
  });
  ...
  // libsodium.js does not export crypto_xof_turboshake128_DOMAIN_STANDARD, so
  ...
  group('methods', () {
    setUp(() {
      when(() => mockSodium.crypto_xof_turboshake128_BLOCKBYTES).thenReturn(5);
      when(() => mockSodium.crypto_xof_turboshake128_STATEBYTES).thenReturn(5);
    });
    ...
    group('call', () {
      test('asserts if outLen is invalid', () {
        ...
        verifyNever(() => mockSodium.crypto_xof_turboshake128(any(), any()));
      });

      test('calls crypto_xof_turboshake128 with correct arguments', () {
        when(() => mockSodium.crypto_xof_turboshake128(any(), any()))
        ...
        verify(
          () => mockSodium.crypto_xof_turboshake128(
            outLen,
    ...
    group('createConsumer', () {
      test('asserts if domain is invalid', () {
        ...
        verifyNever(
          () => mockSodium.crypto_xof_turboshake128_init_with_domain(any()),
        );
      });

      test('creates a consumer that is initialized with '
          'crypto_xof_turboshake128_init', () {
        when(() => mockSodium.crypto_xof_turboshake128_init())
        ...
        expect(result, isA<XofConsumerJS<XofTurboshake128State>>());

        verify(() => mockSodium.crypto_xof_turboshake128_init());
        verifyNever(
          () => mockSodium.crypto_xof_turboshake128_init_with_domain(any()),
        );
      });
      ...
      test('creates a consumer that is initialized with '
        ...
        when(() => mockSodium.crypto_xof_turboshake128_init_with_domain(any()))
            .thenReturn(state.toJS);
        ...
        verify(
          () => mockSodium.crypto_xof_turboshake128_init_with_domain(domain),
        );
        verifyNever(() => mockSodium.crypto_xof_turboshake128_init());
      });
      ...
      test('creates a consumer that uses crypto_xof_turboshake128_update and '
        ...
        when(() => mockSodium.crypto_xof_turboshake128_squeeze(any(), any()))
            .thenReturn(Uint8List.fromList(output).toJS);
        ...
        verifyInOrder([
          () => mockSodium.crypto_xof_turboshake128_update(
            state.toJS,
            Uint8List.fromList(message).toJS,
          ),
          () => mockSodium.crypto_xof_turboshake128_squeeze(state.toJS, outLen),
        ]);
      });
    });
  });
