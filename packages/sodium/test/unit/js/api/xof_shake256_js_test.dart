...
void main() {
  ...
  setUp(() {
    ...
    sut = XofShake256JS(mockSodium.asLibSodiumJS);
  });
  ...
  // libsodium.js does not export crypto_xof_shake256_DOMAIN_STANDARD, so the
  // value is hardcoded in the implementation instead of being read from the
  // bindings.
  ...
  group('methods', () {
    setUp(() {
      ...
      when(() => mockSodium.crypto_xof_shake256_STATEBYTES).thenReturn(5);
    });
    ...
    group('call', () {
      test('asserts if outLen is invalid', () {
        ...
        verifyNever(() => mockSodium.crypto_xof_shake256(any(), any()));
      });

      test('calls crypto_xof_shake256 with correct arguments', () {
        when(() => mockSodium.crypto_xof_shake256(any(), any()))
    ...
    group('createConsumer', () {
      test('asserts if domain is invalid', () {
        ...
        verifyNever(
          () => mockSodium.crypto_xof_shake256_init_with_domain(any()),
        );
      });

      test('creates a consumer that is initialized with '
          'crypto_xof_shake256_init', () {
        when(() => mockSodium.crypto_xof_shake256_init())
        ...
        expect(result, isA<XofConsumerJS<XofShake256State>>());

        verify(() => mockSodium.crypto_xof_shake256_init());
        verifyNever(
          () => mockSodium.crypto_xof_shake256_init_with_domain(any()),
        );
      });
      ...
      test('creates a consumer that is initialized with '
        ...
        when(() => mockSodium.crypto_xof_shake256_init_with_domain(any()))
            .thenReturn(state.toJS);
        ...
        verify(() => mockSodium.crypto_xof_shake256_init_with_domain(domain));
        verifyNever(() => mockSodium.crypto_xof_shake256_init());
      });
      ...
      test('creates a consumer that uses crypto_xof_shake256_update and '
        ...
        when(() => mockSodium.crypto_xof_shake256_squeeze(any(), any()))
            .thenReturn(Uint8List.fromList(output).toJS);
        ...
        verifyInOrder([
          ...
          () => mockSodium.crypto_xof_shake256_squeeze(state.toJS, outLen),
        ]);
      });
    });
  });
