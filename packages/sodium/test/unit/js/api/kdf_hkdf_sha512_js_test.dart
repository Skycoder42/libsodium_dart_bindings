...
void main() {
  ...
  setUp(() {
    ...
    sut = KdfHkdfSha512JS(mockSodium.asLibSodiumJS);
  });
  ...
  group('methods', () {
    ...
    group('expand', () {
      ...
      test('calls crypto_kdf_hkdf_sha512_expand with correct arguments', () {
        when(
          () => mockSodium.crypto_kdf_hkdf_sha512_expand(any(), any(), any()),
        ).thenReturn(Uint8List(0).toJS);
    ...
    group('createExtractConsumer', () {
      test('returns KdfHkdfExtractConsumerJS and wires extract_init', () {
        ...
        verify(() => mockSodium.crypto_kdf_hkdf_sha512_extract_init(null));
      });
    ...
    group('extractStream', () {
      test('extracts master key using the incremental sha512 apis', () async {
        ...
        verifyInOrder([
          () => mockSodium.crypto_kdf_hkdf_sha512_extract_init(null),
          () => mockSodium.crypto_kdf_hkdf_sha512_extract_update(
            state.toJS,
            Uint8List.fromList(ikm).toJS,
          ),
          () => mockSodium.crypto_kdf_hkdf_sha512_extract_final(state.toJS),
        ]);
      });
    });
  });
