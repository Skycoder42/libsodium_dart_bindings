...
import 'package:sodium/src/ffi/bindings/libsodium.ffi.dart'
    show crypto_xof_turboshake256_state;
...
void main() {
  ...
  setUp(() {
    ...
    sut = XofTurboshake256FFI(mockSodium);
  });
  ...
  testConstantsMapping([
    ...
    (
      () => mockSodium.crypto_xof_turboshake256_domain_standard(),
      () => sut.domainStandard,
      'domainStandard',
    ),
  ]);
  ...
  group('methods', () {
    setUp(() {
      when(() => mockSodium.crypto_xof_turboshake256_blockbytes())
          .thenReturn(5);
      when(() => mockSodium.crypto_xof_turboshake256_statebytes())
          .thenReturn(stateBytes);
      when(() => mockSodium.crypto_xof_turboshake256_domain_standard())
          .thenReturn(0x1F);
    });
    ...
    group('call', () {
      test('asserts if outLen is invalid', () {
        ...
        verifyNever(
          () => mockSodium.crypto_xof_turboshake256(any(), any(), any(), any()),
        );
      });
      ...
      test('throws if crypto_xof_turboshake256 fails', () {
        when(
          () => mockSodium.crypto_xof_turboshake256(any(), any(), any(), any()),
        ).thenReturn(1);
    ...
    group('createConsumer', () {
      test('asserts if domain is invalid', () {
        ...
        verifyNever(
          () => mockSodium.crypto_xof_turboshake256_init_with_domain(
            any(),
            any(),
          ),
        );
      ...
      test('creates a consumer that is initialized with '
        ...
        when(() => mockSodium.crypto_xof_turboshake256_init(any()))
            .thenReturn(0);
        ...
        expect(result, isA<XofConsumerFFI<crypto_xof_turboshake256_state>>());
      ...
      test('creates a consumer that is initialized with '
        ...
        verifyNever(() => mockSodium.crypto_xof_turboshake256_init(any()));
      });
      ...
      test('creates a consumer that uses crypto_xof_turboshake256_update and '
        ...
        when(() => mockSodium.crypto_xof_turboshake256_init(any()))
            .thenReturn(0);
        when(
          () => mockSodium.crypto_xof_turboshake256_update(any(), any(), any()),
        ).thenReturn(0);

        final output = List.generate(outLen, (index) => index * 2);
        when(
          () =>
              mockSodium.crypto_xof_turboshake256_squeeze(any(), any(), any()),
        ).thenAnswer((i) {
        ...
        verifyInOrder([
          ...
          () => mockSodium.crypto_xof_turboshake256_squeeze(
            any(that: isNot(nullptr)),
            any(that: isNot(nullptr)),
            outLen,
          ),
