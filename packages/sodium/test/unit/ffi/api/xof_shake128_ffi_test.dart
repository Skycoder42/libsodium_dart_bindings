...
void main() {
  ...
  setUp(() {
    ...
    sut = XofShake128FFI(mockSodium);
  });

  testConstantsMapping([
    (
      () => mockSodium.crypto_xof_shake128_blockbytes(),
      () => sut.blockBytes,
      'blockBytes',
    ),
    (
      () => mockSodium.crypto_xof_shake128_statebytes(),
      () => sut.stateBytes,
      'stateBytes',
    ),
    (
      () => mockSodium.crypto_xof_shake128_domain_standard(),
      () => sut.domainStandard,
      'domainStandard',
    ),
  ]);
  ...
  group('methods', () {
    setUp(() {
      when(() => mockSodium.crypto_xof_shake128_blockbytes()).thenReturn(5);
      when(() => mockSodium.crypto_xof_shake128_statebytes())
          .thenReturn(stateBytes);
      when(() => mockSodium.crypto_xof_shake128_domain_standard())
          .thenReturn(0x1F);
    });
    ...
    group('call', () {
      test('asserts if outLen is invalid', () {
        ...
        verifyNever(
          () => mockSodium.crypto_xof_shake128(any(), any(), any(), any()),
        );
      });

      test('calls crypto_xof_shake128 with correct arguments', () {
        when(() => mockSodium.crypto_xof_shake128(any(), any(), any(), any()))
            .thenReturn(0);
        ...
        verifyInOrder([
          ...
          () => mockSodium.crypto_xof_shake128(
      ...
      test('throws if crypto_xof_shake128 fails', () {
        when(() => mockSodium.crypto_xof_shake128(any(), any(), any(), any()))
            .thenReturn(1);
    ...
    group('createConsumer', () {
      ...
      test('creates a consumer that is initialized with '
        ...
        expect(result, isA<XofConsumerFFI<crypto_xof_shake128_state>>());

        verifyInOrder([
          () => mockSodium.crypto_xof_shake128_statebytes(),
          () => mockSodium.sodium_allocarray(stateBytes, 1),
          () => mockSodium.crypto_xof_shake128_init(any(that: isNot(nullptr))),
        ]);
        verifyNever(
          () => mockSodium.crypto_xof_shake128_init_with_domain(any(), any()),
        );
      ...
      test('creates a consumer that uses crypto_xof_shake128_update and '
