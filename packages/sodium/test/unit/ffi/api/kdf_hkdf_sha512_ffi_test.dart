...
void main() {
  ...
  setUp(() {
    ...
    sut = KdfHkdfSha512FFI(mockSodium);
  });

  testConstantsMapping([
    (
      () => mockSodium.crypto_kdf_hkdf_sha512_keybytes(),
      () => sut.keyBytes,
      'keyBytes',
    ),
    (
      () => mockSodium.crypto_kdf_hkdf_sha512_bytes_min(),
      () => sut.bytesMin,
      'bytesMin',
    ),
    (
      () => mockSodium.crypto_kdf_hkdf_sha512_bytes_max(),
      () => sut.bytesMax,
      'bytesMax',
    ),
  ]);
  ...
  group('methods', () {
    setUp(() {
      when(() => mockSodium.crypto_kdf_hkdf_sha512_keybytes()).thenReturn(5);
      when(() => mockSodium.crypto_kdf_hkdf_sha512_bytes_min()).thenReturn(5);
      when(() => mockSodium.crypto_kdf_hkdf_sha512_bytes_max()).thenReturn(15);
      when(() => mockSodium.crypto_kdf_hkdf_sha512_statebytes()).thenReturn(5);
    });

    testKeygen(
      mockSodium: mockSodium,
      runKeygen: () => sut.keygen(),
      keyBytesNative: mockSodium.crypto_kdf_hkdf_sha512_keybytes,
      keygenNative: mockSodium.crypto_kdf_hkdf_sha512_keygen,
    );

    group('extract', () {
      test('calls crypto_kdf_hkdf_sha512_extract with correct arguments', () {
        when(
          () => mockSodium.crypto_kdf_hkdf_sha512_extract(
            any(),
            any(),
            any(),
            any(),
      ...
      test('throws if crypto_kdf_hkdf_sha512_extract fails', () {
        when(
          () => mockSodium.crypto_kdf_hkdf_sha512_extract(
            any(),
            any(),
    ...
    group('expand', () {
      test('asserts if masterKey is invalid', () {
        ...
        verify(() => mockSodium.crypto_kdf_hkdf_sha512_keybytes());
      });
      ...
      test('asserts if outLen is invalid', () {
        ...
        verify(() => mockSodium.crypto_kdf_hkdf_sha512_bytes_min());
        verify(() => mockSodium.crypto_kdf_hkdf_sha512_bytes_max());
      });

      test('calls crypto_kdf_hkdf_sha512_expand with correct arguments', () {
        when(
          () => mockSodium.crypto_kdf_hkdf_sha512_expand(
            any(),
            any(),
            any(),
            any(),
      ...
      test('throws if crypto_kdf_hkdf_sha512_expand fails', () {
        when(
          () => mockSodium.crypto_kdf_hkdf_sha512_expand(
            any(),
            any(),
    ...
    group('createExtractConsumer', () {
      test('returns KdfHkdfExtractConsumerFFI and wires extract_init', () {
        when(
          () => mockSodium.crypto_kdf_hkdf_sha512_extract_init(
            any(),
            any(),
            any(),
          ),
    ...
    group('extractStream', () {
      test('extracts master key using the incremental sha512 apis', () async {
        ...
        when(
          () => mockSodium.crypto_kdf_hkdf_sha512_extract_update(
            any(),
            any(),
            any(),
          ),
        ...
        when(
          () => mockSodium.crypto_kdf_hkdf_sha512_extract_final(any(), any()),
        ).thenAnswer((i) {
        ...
        verifyInOrder([
          ...
          () => mockSodium.crypto_kdf_hkdf_sha512_extract_final(
            any(that: isNot(nullptr)),
            any(that: isNot(nullptr)),
          ),
        ]);
